...
export class SubtitleStyler {
  ...
  private keywordStyle: SubtitleStyle;
  private codeStyle: SubtitleStyle;
   ...
   * 키워드 강조 스타일
   */
  private createKeywordStyle(): SubtitleStyle {
    return {
      ...this.defaultStyle,
   ...
   * 코드 블록 스타일
   */
  private createCodeStyle(): SubtitleStyle {
    return {
      fontSize: 20,
   ...
   * 텍스트 타입별 스타일 적용
   */
  public getStyleForText(_text: string, type: TextType = 'normal'): SubtitleStyle {
    switch (type) {
      case 'keyword':
   ...
   * 제목 스타일
   */
  private getTitleStyle(): SubtitleStyle {
    return {
      ...this.defaultStyle,
      fontSize: 36,
      fontFamily: "'Noto Sans KR', sans-serif",
   ...
   * 캡션 스타일
   */
  private getCaptionStyle(): SubtitleStyle {
    return {
      ...this.defaultStyle,
      fontSize: 18,
      position: {
   ...
   * CSS 스타일 생성
   */
  public generateCSS(style: SubtitleStyle): string {
    const css: string[] = [];
   ...
   * 애니메이션 CSS 생성
   */
  public generateAnimationCSS(animation: SubtitleAnimation): string {
   ...
   * 애니메이션별 키프레임 정의
   */
  private getKeyframesForAnimation(type: AnimationType): string {
   ...
   * 반응형 폰트 크기 계산
   */
  public getResponsiveFontSize(baseSize: number, viewportWidth: number): number {
    const minWidth = 320;
    const maxWidth = 1920;
    const minFontSize = baseSize * 0.6;
    const maxFontSize = baseSize * 1.2;

    if (viewportWidth <= minWidth) return minFontSize;
    if (viewportWidth >= maxWidth) return maxFontSize;
   ...
   * 키워드 하이라이팅
   */
  public highlightKeywords(text: string, keywords: string[]): string {
   ...
   * 스타일 프리셋
   */
  public getPreset(presetName: PresetName): SubtitleStyle {
    const presets: Record<PresetName, SubtitleStyle> = {
      'minimal': {
        ...this.defaultStyle,
