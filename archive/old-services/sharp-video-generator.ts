...
export class SharpVideoGenerator {
  ...
  private async createSceneFrames(scene: Scene, sceneIndex: number, startFrameIndex: number): Promise<string[]> {
  ...
  private createIntroSVG(scene: Scene, progress: number): string {
  ...
  private createConceptSVG(scene: Scene, progress: number): string {
  ...
  private createProcessSVG(scene: Scene, progress: number): string {
  ...
  private createBenefitsSVG(scene: Scene, progress: number): string {
  ...
  private createExampleSVG(scene: Scene, progress: number): string {
  ...
  private createConclusionSVG(scene: Scene, progress: number): string {
    const fadeIn = Math.min(progress * 2, 1);
  ...
  private createDefaultSVG(scene: Scene, progress: number): string {
    return `
      <svg width="${this.width}" height="${this.height}" xmlns="http://www.w3.org/2000/svg">
        <rect width="${this.width}" height="${this.height}" fill="#1e293b"/>
  ...
  private async getScenario(): Promise<any> {
  ...
  private parseScenes(scenesData: any): Scene[] {
  ...
  private async createVideo(framePaths: string[]): Promise<string> {
  ...
  private async updateVideoStatus(status: string, videoPath?: string, error?: string) {
...
export async function startSharpVideoGeneration(
  videoId: string,
  projectId: string,
  scenarioId?: string
) {
