...
// IT 교육용 템플릿
export const templates: Record<string, ScenarioTemplate> = {
  // 1. 개념 설명 템플릿
  conceptExplanation: {
    name: '개념 설명',
...
// 시나리오 생성 가이드라인
export const scenarioGuidelines = {
  // 좋은 시나리오의 특징
  bestPractices: [
    '명확한 학습 목표 설정',
    '단계적 난이도 상승',
...
// 시나리오 품질 평가 함수
export function evaluateScenarioQuality(scenario: any): {
  score: number;
  feedback: string[];
  improvements: string[];
} {
  ...
  if (scenario.scenes?.some((s: any) => s.type === 'intro') && 
      scenario.scenes?.some((s: any) => s.type === 'conclusion')) {
    score += 10;
  ...
  const totalDuration = scenario.scenes?.reduce((sum: number, s: any) => sum + (s.duration || 0), 0) || 0;
  ...
  // 2. 내용 평가 (40점)
  const hasNarration = scenario.scenes?.every((s: any) => s.narration && s.narration.length > 10);
  if (hasNarration) {
    score += 20;
  ...
  const hasVisuals = scenario.scenes?.every((s: any) => s.title || s.visuals);
  if (hasVisuals) {
    score += 20;
  ...
  // 3. 다양성 평가 (30점)
  const sceneTypes = new Set(scenario.scenes?.map((s: any) => s.type));
...
// 시나리오 자동 개선 함수
export function enhanceScenario(scenario: any, template?: string): any {
  const selectedTemplate = template ? templates[template] : templates.conceptExplanation;
  ...
  // 기존 시나리오 개선
  const enhancedScenes = scenario.scenes.map((scene: any, index: number) => {
