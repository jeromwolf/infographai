...
class TemplateLoader {
    ...
    constructor() {
        ...
        this.assetsPath = path_1.default.resolve(__dirname, '../../../../assets');
    }
    /**
     * 템플릿 파일을 로드하고 캐싱
     */
    async loadTemplate(templatePath) {
        ...
        try {
            const fullPath = path_1.default.join(this.assetsPath, templatePath);
            const templateContent = await promises_1.default.readFile(fullPath, 'utf-8');
     ...
     * 템플릿에 동적 데이터를 삽입
     */
    async renderTemplateWithData(templatePath, data) {
    ...
    injectData(template, data) {
        ...
        if (data.steps && Array.isArray(data.steps)) {
            const stepsHtml = data.steps.map((step, index) => `<text x="50" y="${100 + index * 40}" class="step-text">${index + 1}. ${this.escapeHtml(step)}</text>`).join('\n');
     ...
     */
    static createTemplateData(type, sceneData) {
        switch (type) {
            case 'code':
                return {
                    title: sceneData.title,
                    subtitle: sceneData.subtitle,
                    code: sceneData.visualElements?.find((el) => el.type === 'code_block')?.content || '',
                    language: sceneData.visualElements?.find((el) => el.type === 'code_block')?.style?.language || 'python'
                };
            case 'process':
                return {
                    title: sceneData.title,
                    subtitle: sceneData.subtitle,
                    steps: sceneData.visualElements?.filter((el) => el.type === 'step' || el.type === 'process_step')
                        .map((el) => el.content.replace(/\d+\.\s*/, '').replace(/\n/g, ' ')) || []
                };
            case 'chart':
                return {
                    title: sceneData.title,
                    subtitle: sceneData.subtitle,
                    data: sceneData.dataPoints || sceneData.visualElements?.filter((el) => el.type === 'data') || []
                };
            default:
                return {
                    title: sceneData.title,
