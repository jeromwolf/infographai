...
exports.AutoScenarioGenerator = void 0;
...
class TopicAnalyzer {
    ...
    selectSceneStructure(category, difficulty) {
...
class ContentGenerator {
    async generateSceneContent(topic, sceneTemplate, context) {
    ...
    generateTitle(topic, sceneTemplate) {
        return sceneTemplate.template.replace('{topic}', topic);
    }
    async generateContent(topic, template, context) {
        // 규칙 기반 생성
    ...
    hasRuleBasedPattern(topic, type) {
        const rulePatterns = {
    ...
    generateByRules(topic, template, context) {
        const rulePatterns = {
            'RAG': {
    ...
    generateByTemplate(topic, template, context) {
    ...
    selectVisualElements(visualStyle, context) {
        // 시각적 요소 선택 로직 (추후 구현)
        return {
            style: visualStyle,
...
// 스타일 선택기 클래스
class StyleSelector {
    selectSceneStyle(sceneType, category, sceneIndex, totalScenes) {
        // 카테고리별 색상 팔레트
        const colorPalettes = {
...
class AutoScenarioGenerator {
    ...
    async generateFromTopic(topic, userId) {
        ...
        for (let i = 0; i < sceneTemplates.length; i++) {
            ...
            // 내용 생성
            const content = await this.contentGenerator.generateSceneContent(topic, template, analysis);
            // 스타일 선택
            const style = this.styleSelector.selectSceneStyle(template.type, analysis.category, i, sceneTemplates.length);
            // 씬 완성
            scenes.push({
                id: `scene_${i + 1}`,
                type: template.type,
    ...
    // 데이터베이스에 시나리오 저장
    async saveToDatabase(scenario, projectId) {
        try {
...
exports.AutoScenarioGenerator = AutoScenarioGenerator;
