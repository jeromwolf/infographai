...
export class SmartTemplateSelector {
  private templateRules: Map<string, TemplateRule>;
   ...
   * 주제별 템플릿 매칭 규칙 초기화
   */
  private initializeRules(): Map<string, TemplateRule> {
    const rules = new Map<string, TemplateRule>();
   ...
   * 씬에 대한 최적 템플릿 선택
   */
  public async selectTemplatesForScene(
    scene: EnhancedScene,
    topic: string,
   ...
   * 키워드 추출
   */
  private extractKeywords(topic: string, scene: EnhancedScene): string[] {
   ...
   * 매칭 규칙 찾기
   */
  private findMatchingRules(keywords: string[]): TemplateRule[] {
    const matches: { rule: TemplateRule; score: number }[] = [];
    ...
    this.templateRules.forEach((rule, key) => {
      ...
      for (const keyword of keywords) {
        if (rule.keywords.some(ruleKeyword =>
          keyword.includes(ruleKeyword) || ruleKeyword.includes(keyword)
        )) {
          score += rule.priority;
        }
      }
   ...
   * 템플릿 조합 생성
   */
  private async composeTemplates(
    scene: EnhancedScene,
    matchedRules: TemplateRule[],
    ...
    switch (scene.type) {
      ...
      case 'example':
        if (matchedRules.length > 0) {
          ...
          const codeTemplate = templates.find(t =>
            t.includes('code-editors') || t.includes('terminals')
          );
   ...
   * 애니메이션 및 트랜지션 강화
   */
  private enhanceWithAnimations(composition: TemplateComposition, scene: EnhancedScene): void {
    // 씬 애니메이션 추가
    if (scene.animation) {
  ...
  public async recommendTemplates(
    topic: string,
    sceneCount: number = 6
  ): Promise<Map<string, TemplateComposition>> {
    const recommendations = new Map<string, TemplateComposition>();

    // 기본 씬 타입들
    const sceneTypes = ['intro', 'concept', 'process', 'benefits', 'example', 'conclusion'];

    for (let i = 0; i < Math.min(sceneCount, sceneTypes.length); i++) {
      const mockScene: EnhancedScene = {
        id: i + 1,
        type: sceneTypes[i] as any,
      ...
      const composition = await this.selectTemplatesForScene(
        mockScene,
        topic,
        'intermediate'
      );
   ...
   * 템플릿 사용 통계
   */
  public getTemplateUsageStats(): Map<string, number> {
