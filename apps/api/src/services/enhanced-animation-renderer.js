...
exports.enhancedAnimationRenderer = exports.EnhancedAnimationRenderer = void 0;
const sharp_1 = __importDefault(require("sharp"));
const smart_template_selector_1 = require("./smart-template-selector");
class EnhancedAnimationRenderer {
     ...
     * 씬 렌더링 with 스마트 템플릿
     */
    async renderSceneWithTemplates(scene, frameNumber, totalFrames, topic, userLevel = 'intermediate') {
        // 1. 템플릿 선택
        const templates = await smart_template_selector_1.smartTemplateSelector.selectTemplatesForScene(scene, topic, userLevel);
        // 2. 애니메이션 컨텍스트 생성
        const context = {
     ...
     * 레이어 구성
     */
    async composeLayers(scene, context) {
     ...
     * 레이어 합성
     */
    async compositeLayers(layers, context) {
        // Base canvas
        let composite = (0, sharp_1.default)({
            create: {
                width: this.width,
                height: this.height,
                channels: 4,
     ...
     * 개별 레이어 렌더링
     */
    async renderLayer(layerName, config, context) {
        switch (config.type) {
            case 'background':
     ...
     * 배경 레이어 렌더링
     */
    async renderBackgroundLayer(config) {
        const svg = `
     ...
     * 템플릿 레이어 렌더링
     */
    async renderTemplateLayer(layerName, config, context) {
        try {
            let templatePath = '';
            ...
            const templateContent = await smart_template_selector_1.smartTemplateSelector.loadTemplate(templatePath);
            // Apply animations based on progress
            const animatedContent = this.applyTemplateAnimations(templateContent, context);
            return (0, sharp_1.default)(Buffer.from(animatedContent))
                .resize(config.size.width, config.size.height)
                .png()
                .toBuffer();
        }
     ...
     * 애니메이션 레이어 렌더링
     */
    async renderAnimationLayer(config, context) {
     ...
     * 콘텐츠 레이어 렌더링
     */
    async renderContentLayer(config, context) {
     ...
     * 오버레이 레이어 렌더링
     */
    async renderOverlayLayer(config, context) {
        const svg = `
      <svg width="${config.size.width}" height="${config.size.height}" xmlns="http://www.w3.org/2000/svg">
     ...
     * 애니메이션 불투명도 계산
     */
    calculateAnimationOpacity(context) {
     ...
     * 템플릿 애니메이션 적용
     */
    applyTemplateAnimations(templateContent, context) {
     ...
     * 애니메이션 SVG 생성
     */
    generateAnimationSVG(animationType, progress, config) {
     ...
     * 컨텍스트 기반 트랜지션 생성
     */
    async createTransition(fromScene, toScene, frameNumber, transitionDuration = 30 // 1 second at 30fps
    ) {
     ...
     * 트랜지션 타입 선택
     */
    selectTransitionType(fromType, toType) {
     ...
     * 트랜지션 SVG 생성
     */
    generateTransitionSVG(transitionType, progress) {
...
exports.EnhancedAnimationRenderer = EnhancedAnimationRenderer;
// Export singleton instance
exports.enhancedAnimationRenderer = new EnhancedAnimationRenderer();
