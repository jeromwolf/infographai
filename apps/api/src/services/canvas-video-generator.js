...
exports.CanvasVideoGenerator = void 0;
exports.startCanvasVideoGeneration = startCanvasVideoGeneration;
...
class CanvasVideoGenerator {
    ...
    async getScenario() {
        ...
        return await database_1.prisma.scenario.findFirst({
            where: { projectId: this.projectId },
            orderBy: { createdAt: 'desc' }
        });
    }
    async createSceneFrames(scene, sceneIndex) {
    ...
    async drawIntroScene(ctx, scene, progress) {
        // Gradient background
    ...
    async drawConceptScene(ctx, scene, progress) {
        // Background
    ...
    async drawProcessScene(ctx, scene, progress) {
        // Background
    ...
    async drawBenefitsScene(ctx, scene, progress) {
        // Background gradient
    ...
    async drawExampleScene(ctx, scene, progress) {
        // Background
    ...
    async drawConclusionScene(ctx, scene, progress) {
        // Gradient background
        const gradient = ctx.createRadialGradient(this.width / 2, this.height / 2, 0, this.width / 2, this.height / 2, this.width / 2);
    ...
    async drawDefaultScene(ctx, scene, progress) {
    ...
    async createVideoFromFrames(framePaths) {
    ...
    async updateVideoStatus(status, outputUrl, error) {
        const updateData = {
...
exports.CanvasVideoGenerator = CanvasVideoGenerator;
async function startCanvasVideoGeneration(videoId, projectId, scenarioId) {
