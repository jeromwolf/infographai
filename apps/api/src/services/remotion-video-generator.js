...
exports.RemotionVideoGenerator = void 0;
exports.startRemotionVideoGeneration = startRemotionVideoGeneration;
...
class RemotionVideoGenerator {
    outputDir;
    constructor() {
        this.outputDir = path_1.default.join(process.cwd(), 'public', 'videos');
    }
    /**
     * Remotion을 사용하여 비디오 생성
     */
    async generate(videoId, projectId, scenarioId) {
        try {
     ...
     * 시나리오 데이터 조회
     */
    async getScenarioData(scenarioId, projectId) {
        if (scenarioId) {
     ...
     */
    async convertToRemotionProps(scenarioData) {
        ...
        const remotionScenes = scenes.map((scene, index) => ({
     ...
     */
    async renderWithRemotion(videoId, props) {
        ...
        try {
            ...
            */
            // 임시: 더미 파일 생성 (테스트용)
            await promises_1.default.writeFile(outputPath, Buffer.from('Mock video content for ' + videoId));
     ...
     * 비디오 생성 완료 처리
     */
    async finalizeVideo(videoId, videoPath) {
     ...
     * 비디오 상태 업데이트
     */
    async updateVideoStatus(videoId, status, error) {
        await prisma.video.update({
            where: { id: videoId },
            data: {
                status,
...
exports.RemotionVideoGenerator = RemotionVideoGenerator;
/**
 * 비디오 생성 시작 함수 (기존 API와의 호환성 유지)
 */
async function startRemotionVideoGeneration(videoId, projectId, scenarioId) {
