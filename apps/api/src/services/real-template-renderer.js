...
exports.RealTemplateRenderer = void 0;
...
class RealTemplateRenderer {
    ...
    async renderWithRealTemplate(scene, progress) {
        // 템플릿 경로 결정
    ...
    getTemplatePath(scene) {
        ...
        switch (scene.type) {
            ...
            case 'example':
                ...
                const hasCode = scene.visualElements?.some(el => el.type === 'code' || el.type === 'code_block');
                if (hasCode) {
    ...
    prepareTemplateData(scene, progress) {
        const data = {
            title: scene.title || '',
    ...
    applyAnimations(svg, scene, progress) {
        if (!scene.animations)
            return svg;
        let result = svg;
    ...
    applyTypewriterEffect(svg, animation, progress) {
    ...
    applySlideAnimation(svg, animation, progress) {
    ...
    applyFadeAnimation(svg, animation, progress) {
        const animationStart = (animation.delay || 0) / 5;
        const animationEnd = animationStart + (animation.duration || 1) / 5;
        if (progress < animationStart) {
    ...
    applyZoomAnimation(svg, animation, progress) {
        const animationStart = (animation.delay || 0) / 5;
        const animationEnd = animationStart + (animation.duration || 1) / 5;
        if (progress < animationStart) {
    ...
    applyBounceAnimation(svg, animation, progress) {
        const animationStart = (animation.delay || 0) / 5;
        const animationEnd = animationStart + (animation.duration || 1) / 5;
        if (progress < animationStart) {
    ...
    async renderFallback(scene, progress) {
        // 기본 SVG 생성
        const svg = `
...
exports.RealTemplateRenderer = RealTemplateRenderer;
