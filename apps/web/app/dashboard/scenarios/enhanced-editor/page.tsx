...
import { 
  Sparkles, Play, Pause, RotateCcw, Download, Upload,
  Film, Type, Image, Music, Mic, Settings, ChevronRight,
  Clock, FileText, Wand2, Brain, BookOpen, Target,
  Layers, Grid, CheckCircle, AlertCircle
} from 'lucide-react';
...
const aiPromptTemplates: AIPromptTemplate[] = [
  {
    id: 'educational',
    name: '교육용 콘텐츠',
    icon: BookOpen,
...
export default function EnhancedScenarioEditor() {
  const [scenes, setScenes] = useState<Scene[]>([]);
  const [selectedScene, setSelectedScene] = useState<Scene | null>(null);
  const [activeTab, setActiveTab] = useState<'script' | 'visual' | 'audio'>('script');
  const [isGenerating, setIsGenerating] = useState(false);
  const [showAIPanel, setShowAIPanel] = useState(true);
  const [aiInput, setAiInput] = useState('');
  const [selectedTemplate, setSelectedTemplate] = useState<AIPromptTemplate>(aiPromptTemplates[0]);
  const [projectTitle, setProjectTitle] = useState('');
  const [targetAudience, setTargetAudience] = useState('개발자 및 IT 전문가');
  ...
  const updateScene = (updates: Partial<Scene>) => {
    ...
    const updatedScenes = scenes.map(s => 
      s.id === selectedScene.id ? { ...s, ...updates } : s
    );
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          placeholder="이 씬에서 설명할 내용을 작성하세요..."
                        />
                      </div>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <select
                            ...
                            onChange={(e) => updateScene({ type: e.target.value as Scene['type'] })}
                            className="w-full p-3 border rounded-lg"
                          >
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          {['modern-gradient', 'minimal', 'tech', 'dark', 'colorful', 'professional'].map(style => (
                            <button
                              key={style}
                              onClick={() => updateScene({ 
                              ...
                              className={`p-3 rounded-lg border text-center ${
                                selectedScene.visuals?.style === style
                                  ? 'border-blue-500 bg-blue-50'
                                  : 'border-gray-200 hover:border-gray-300'
                              }`}
                            >
                      ...
                      <div>
                        ...
                        <select
                          value={selectedScene.visuals?.animation || 'fade-in'}
                  ...
                  {activeTab === 'audio' && (
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {['친근한', '전문적인', '차분한', '활기찬'].map(style => (
                            <button
                              ...
                              className={`p-3 rounded-lg border text-center ${
                                selectedScene.audio?.voiceStyle === style
                                  ? 'border-blue-500 bg-blue-50'
                                  : 'border-gray-200 hover:border-gray-300'
                              }`}
                            >
                              <Mic className="w-5 h-5 mx-auto mb-1 text-blue-600" />
                      ...
                      <div>
                        ...
                        <select
                          value={selectedScene.audio?.bgm || 'none'}
                          onChange={(e) => updateScene({ 
