...
function SceneBuilder({ initialCommands, onSave } = {}) {
    ...
    const [draggedAsset, setDraggedAsset] = (0, react_1.useState)(null);
    const [isPreviewMode, setIsPreviewMode] = (0, react_1.useState)(false);
    const [exportCode, setExportCode] = (0, react_1.useState)('');
    // KodeKloud colors
    const COLORS = {
        bg: '#0a0a0a',
        primary: '#4ade80',
        ...
        elements.forEach(element => {
            ...
            switch (element.type) {
                ...
                case 'arrow':
                    ...
                    const headLength = 12;
                    ctx.beginPath();
                    ctx.moveTo(element.x + element.props.x2, element.y + element.props.y2);
                    ctx.lineTo(element.x + element.props.x2 - headLength * Math.cos(angle - Math.PI / 6), element.y + element.props.y2 - headLength * Math.sin(angle - Math.PI / 6));
                    ctx.moveTo(element.x + element.props.x2, element.y + element.props.y2);
                    ctx.lineTo(element.x + element.props.x2 - headLength * Math.cos(angle + Math.PI / 6), element.y + element.props.y2 - headLength * Math.sin(angle + Math.PI / 6));
                    ctx.stroke();
                    break;
    ...
    const updateElementProp = (propName, value) => {
        if (!selectedElement)
            return;
    ...
    const generateCommands = () => {
        return elements.map(element => {
            const command = { type: element.type };
...
/>`;
    ...
    (0, react_1.useEffect)(() => {
        if (initialCommands && initialCommands.length > 0) {
            const newElements = initialCommands.map((cmd, index) => {
    ...
    return (<div className="flex h-screen bg-gray-950">
      ...
      <div className="w-80 bg-gray-900 border-r border-gray-800 p-4 overflow-y-auto">
        <h3 className="text-xl font-bold text-green-400 mb-4" style={{ fontFamily: 'Kalam, cursive' }}>
          KodeKloud Assets
        </h3>
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {AVAILABLE_ASSETS.filter(a => a.id.includes('text') || a.id.includes('title') || a.id.includes('label')).map(asset => (<div key={asset.id} draggable onDragStart={(e) => handleDragStart(e, asset)} className="bg-gray-800 p-3 rounded-lg cursor-move hover:bg-gray-700 transition-colors text-center" title={asset.label}>
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {AVAILABLE_ASSETS.filter(a => (a.type === 'rect' || a.type === 'circle') && !['server', 'database', 'cloud', 'container', 'user', 'api'].includes(a.id)).map(asset => (<div key={asset.id} draggable onDragStart={(e) => handleDragStart(e, asset)} className="bg-gray-800 p-3 rounded-lg cursor-move hover:bg-gray-700 transition-colors text-center" title={asset.label}>
                <span className="text-xl">{asset.icon}</span>
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {AVAILABLE_ASSETS.filter(a => a.type === 'arrow' || a.type === 'line').map(asset => (<div key={asset.id} draggable onDragStart={(e) => handleDragStart(e, asset)} className="bg-gray-800 p-3 rounded-lg cursor-move hover:bg-gray-700 transition-colors text-center" title={asset.label}>
                <span className="text-xl">{asset.icon}</span>
                <span className="text-[10px] block text-gray-400 mt-1">{asset.label.split(' ')[0]}</span>
              </div>))}
          </div>
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {AVAILABLE_ASSETS.filter(a => ['server', 'database', 'cloud', 'container', 'user', 'api'].includes(a.id)).map(asset => (<div key={asset.id} draggable onDragStart={(e) => handleDragStart(e, asset)} className="bg-gray-800 p-3 rounded-lg cursor-move hover:bg-gray-700 transition-colors" title={asset.label}>
        ...
        <div className="mt-8">
          <h3 className="text-lg font-bold text-blue-400 mb-4" style={{ fontFamily: 'Kalam, cursive' }}>
            Scene Elements
          </h3>

          <div className="space-y-2 max-h-64 overflow-y-auto">
            {elements.map(element => (<div key={element.id} onClick={() => setSelectedElement(element)} className={`p-2 rounded cursor-pointer text-sm ${selectedElement?.id === element.id
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="bg-gray-900 border-b border-gray-800 p-4">
          <div className="flex items-center justify-between">
            <div className="flex gap-2">
              <button onClick={() => setIsPreviewMode(!isPreviewMode)} className={`px-4 py-2 rounded-lg font-bold ${isPreviewMode
            ? 'bg-green-500 text-white'
            : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`} style={{ fontFamily: 'Kalam, cursive' }}>
                {isPreviewMode ? 'Edit Mode' : 'Preview'}
              </button>

              <button onClick={() => setElements([])} className="px-4 py-2 bg-gray-700 text-gray-300 rounded-lg hover:bg-gray-600 font-bold" style={{ fontFamily: 'Kalam, cursive' }}>
                Clear All
              </button>

              <button onClick={exportAsCode} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 font-bold" style={{ fontFamily: 'Kalam, cursive' }}>
                Export Code
              </button>
              {onSave && (<button onClick={handleSave} className="px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600 font-bold" style={{ fontFamily: 'Kalam, cursive' }}>
                  Save Scene
                </button>)}
            </div>

            {selectedElement && (<button onClick={deleteSelectedElement} className="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 font-bold" style={{ fontFamily: 'Kalam, cursive' }}>
                Delete Element
              </button>)}
          </div>
        </div>
        ...
        <div className="flex-1 flex items-center justify-center p-8 bg-gray-950">
          {isPreviewMode ? (<DrawingEngine_1.default commands={generateCommands()} width={900} height={600} speed={1} autoPlay={true} loop={true}/>) : (<canvas ref={canvasRef} width={900} height={600} onClick={handleCanvasClick} onDragOver={handleDragOver} onDrop={handleDrop} className="rounded-lg shadow-2xl cursor-crosshair" style={{ backgroundColor: COLORS.bg }}/>)}
        </div>
      </div>

      {/* Properties Panel */}
      {selectedElement && !isPreviewMode && (<div className="w-64 bg-gray-900 border-l border-gray-800 p-4">
          <h3 className="text-lg font-bold text-yellow-400 mb-4" style={{ fontFamily: 'Kalam, cursive' }}>
            Properties
          </h3>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                setElements(elements.map(el => el.id === selectedElement.id ? { ...el, x: newX } : el));
                setSelectedElement({ ...selectedElement, x: newX });
            }} className="w-20 p-1 bg-gray-800 text-white rounded" placeholder="X"/>
                <input type="number" value={selectedElement.y} onChange={(e) => {
                const newY = parseInt(e.target.value);
                setElements(elements.map(el => el.id === selectedElement.id ? { ...el, y: newY } : el));
                setSelectedElement({ ...selectedElement, y: newY });
            }} className="w-20 p-1 bg-gray-800 text-white rounded" placeholder="Y"/>
              </div>
            </div>
            ...
            {selectedElement.type === 'text' && (<>
                <div>
                  ...
                  <input type="text" value={selectedElement.props.text} onChange={(e) => updateElementProp('text', e.target.value)} className="w-full p-2 bg-gray-800 text-white rounded"/>
                </div>
                <div>
                  <label className="text-sm text-gray-400">Size</label>
                  <input type="number" value={selectedElement.props.size} onChange={(e) => updateElementProp('size', parseInt(e.target.value))} className="w-full p-2 bg-gray-800 text-white rounded"/>
                </div>
              </>)}
            ...
            {(selectedElement.type === 'rect') && (<>
                <div>
                  ...
                  <input type="number" value={selectedElement.props.width} onChange={(e) => updateElementProp('width', parseInt(e.target.value))} className="w-full p-2 bg-gray-800 text-white rounded"/>
                </div>
                <div>
                  <label className="text-sm text-gray-400">Height</label>
                  <input type="number" value={selectedElement.props.height} onChange={(e) => updateElementProp('height', parseInt(e.target.value))} className="w-full p-2 bg-gray-800 text-white rounded"/>
                </div>
              </>)}

            {selectedElement.type === 'circle' && (<div>
                <label className="text-sm text-gray-400">Radius</label>
                <input type="number" value={selectedElement.props.radius} onChange={(e) => updateElementProp('radius', parseInt(e.target.value))} className="w-full p-2 bg-gray-800 text-white rounded"/>
              </div>)}

            <div>
              <label className="text-sm text-gray-400">Color</label>
              <select value={selectedElement.props.color} onChange={(e) => updateElementProp('color', e.target.value)} className="w-full p-2 bg-gray-800 text-white rounded">
            ...
            {(selectedElement.type === 'rect' || selectedElement.type === 'circle') && (<div>
                <label className="flex items-center gap-2 text-sm text-gray-400">
                  <input type="checkbox" checked={selectedElement.props.filled || false} onChange={(e) => updateElementProp('filled', e.target.checked)}/>
                  Filled
                </label>
              </div>)}
          </div>
      ...
      {/* Export Code Modal */}
      {exportCode && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-900 p-6 rounded-lg max-w-2xl max-h-screen overflow-auto">
            ...
            <button onClick={() => setExportCode('')} className="mt-4 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">
              Close
            </button>
          </div>
        </div>)}
