...
exports.default = EnhancedSceneBuilder;
const react_1 = __importStar(require("react"));
const DrawingEngine_1 = __importDefault(require("@/components/animation/DrawingEngine"));
function EnhancedSceneBuilder({ initialCommands = [], onSave }) {
    ...
    const renderCanvas = (0, react_1.useCallback)(() => {
        ...
        elements.forEach(element => {
            ...
            if (selectedElement?.id === element.id) {
                ...
                ctx.lineWidth = 2;
                ctx.setLineDash([5, 5]);
                ctx.strokeRect(element.x - 5, element.y - 5, (element.props?.width || element.props?.radius * 2 || 50) + 10, (element.props?.height || element.props?.radius * 2 || 20) + 10);
                ctx.setLineDash([]);
            }
            ctx.restore();
        });
    ...
    const generateCommands = () => {
        return elements.map(element => {
            const baseCommand = { type: element.type };
            switch (element.type) {
                case 'text':
    ...
    const exportAsGIF = async () => {
        ...
        try {
            ...
            const { default: GIF } = await Promise.resolve().then(() => __importStar(require('gif.js')));
            // Create GIF encoder
            const gif = new GIF({
                workers: 2,
                quality: 10,
    ...
    const renderElementWithAnimation = (ctx, element, progress, animationType) => {
    ...
    return (<div className="relative h-screen bg-gray-950">
      {/* Top Toolbar */}
      <div className="absolute top-0 left-0 right-0 bg-gray-900 border-b border-gray-800 p-3 z-10">
        <div className="flex justify-between items-center">
          <div className="flex gap-2">
            <button onClick={undo} disabled={historyIndex === 0} className="px-3 py-1 bg-gray-800 text-white rounded hover:bg-gray-700 disabled:opacity-50" title="Undo (Cmd+Z)">
              ↶ Undo
            </button>
            <button onClick={redo} disabled={historyIndex === history.length - 1} className="px-3 py-1 bg-gray-800 text-white rounded hover:bg-gray-700 disabled:opacity-50" title="Redo (Cmd+Shift+Z)">
              ↷ Redo
            </button>
            <div className="border-l border-gray-700 mx-2"/>
            <button onClick={() => setShowGrid(!showGrid)} className={`px-3 py-1 rounded ${showGrid ? 'bg-blue-600' : 'bg-gray-800'} text-white hover:bg-blue-700`} title="Toggle Grid (G)">
              Grid
            </button>
            <button onClick={() => setSnapToGrid(!snapToGrid)} className={`px-3 py-1 rounded ${snapToGrid ? 'bg-blue-600' : 'bg-gray-800'} text-white hover:bg-blue-700`}>
              Snap
            </button>
            <button onClick={loadAutoSave} className="px-3 py-1 bg-gray-800 text-white rounded hover:bg-gray-700">
              Load Autosave
            </button>
          </div>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={exportAsJSON} className="px-3 py-1 bg-gray-800 text-white rounded hover:bg-gray-700">
              Export JSON
            </button>
            <button onClick={exportAsGIF} className="px-3 py-1 bg-purple-600 text-white rounded hover:bg-purple-700">
              📱 Export GIF
            </button>
            {onSave && (<button onClick={handleSave} className="px-4 py-1 bg-green-600 text-white rounded hover:bg-green-700 font-semibold" title="Save (Cmd+S)">
                💾 Save
              </button>)}
            <button onClick={() => setIsPreviewMode(!isPreviewMode)} className={`px-4 py-1 rounded font-semibold ${isPreviewMode ? 'bg-blue-600 hover:bg-blue-700' : 'bg-purple-600 hover:bg-purple-700'} text-white`}>
      ...
      <div className="flex h-full pt-14">
        ...
        {!isPreviewMode && (<div className="w-72 bg-gray-900 border-r border-gray-800 p-4 overflow-y-auto">
        ...
        <div className="flex-1 flex items-center justify-center bg-gray-950 relative p-4 overflow-hidden">
          {isPreviewMode ? (<div className="w-full h-full flex items-center justify-center overflow-hidden">
              <div className="relative" style={{ maxWidth: '100%', maxHeight: 'calc(100vh - 300px)' }}>
                <DrawingEngine_1.default commands={generateCommands()} width={1200} height={700} speed={1} autoPlay={true} loop={true}/>
              </div>
            </div>) : (<div className="relative w-full h-full flex items-center justify-center">
              <canvas ref={canvasRef} width={1200} height={700} className="border border-gray-800 bg-black max-w-full" style={{
        ...
        {!isPreviewMode && selectedElement && (<div className="w-72 bg-gray-900 border-l border-gray-800 p-4">
            ...
            <div className="space-y-4">
            ...
            }} className="w-20 px-2 py-1 bg-gray-800 text-white rounded" placeholder="X"/>
                  <input type="number" value={selectedElement.y} onChange={(e) => {
                const updated = { ...selectedElement, y: Number(e.target.value) };
                setElements(elements.map(el => el.id === selectedElement.id ? updated : el));
                setSelectedElement(updated);
            }} className="w-20 px-2 py-1 bg-gray-800 text-white rounded" placeholder="Y"/>
                </div>
              </div>

              {selectedElement.type === 'text' && selectedElement.props.text !== undefined && (<div>
                  ...
                  <input type="text" value={selectedElement.props.text} onChange={(e) => {
                    ...
                    setElements(elements.map(el => el.id === selectedElement.id ? updated : el));
                    setSelectedElement(updated);
                }} className="w-full px-3 py-2 bg-gray-800 text-white rounded" placeholder="Enter text..."/>
                </div>)}

              {selectedElement.type === 'text' && selectedElement.props.size !== undefined && (<div>
                  <label className="text-sm text-gray-400">Font Size</label>
                  <input type="number" value={selectedElement.props.size} onChange={(e) => {
                    const updated = {
                        ...selectedElement,
                    ...
                    setElements(elements.map(el => el.id === selectedElement.id ? updated : el));
                    setSelectedElement(updated);
                }} className="w-full px-3 py-2 bg-gray-800 text-white rounded" min="10" max="100"/>
                </div>)}
              ...
              {selectedElement.animation && (<div className="border-t border-gray-700 pt-4">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <select value={selectedElement.animation.type} onChange={(e) => {
                    const updated = {
                        ...selectedElement,
                    ...
                    <div>
                      ...
                      <input type="number" value={selectedElement.animation.duration} onChange={(e) => {
                    const updated = {
                        ...selectedElement,
                    ...
                    setElements(elements.map(el => el.id === selectedElement.id ? updated : el));
                    setSelectedElement(updated);
                }} className="w-full px-3 py-2 bg-gray-800 text-white rounded" min="100" max="5000" step="100"/>
                    </div>

                    <div>
                      <label className="text-sm text-gray-400">Delay (ms)</label>
                      <input type="number" value={selectedElement.animation.delay} onChange={(e) => {
                    const updated = {
                        ...selectedElement,
                    ...
                    setElements(elements.map(el => el.id === selectedElement.id ? updated : el));
                    setSelectedElement(updated);
                }} className="w-full px-3 py-2 bg-gray-800 text-white rounded" min="0" max="10000" step="100"/>
                    </div>

                    <div>
                      <label className="text-sm text-gray-400">Easing</label>
                      <select value={selectedElement.animation.easing} onChange={(e) => {
                    const updated = {
                        ...selectedElement,
                    ...
                    <div>
                      <label className="flex items-center space-x-2">
                        <input type="checkbox" checked={selectedElement.animation.loop} onChange={(e) => {
                    const updated = {
                        ...selectedElement,
              ...
              <button onClick={deleteSelectedElement} className="w-full px-3 py-2 bg-red-600 text-white rounded hover:bg-red-700">
                Delete Element
              </button>
            </div>
          </div>)}
      ...
      {/* Notification Toast */}
      {notification && (<div className={`fixed bottom-4 right-4 px-4 py-2 rounded-lg text-white shadow-lg z-50 ${notification.type === 'success' ? 'bg-green-600' :
