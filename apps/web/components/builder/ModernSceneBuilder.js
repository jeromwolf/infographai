...
exports.default = ModernSceneBuilder;
const react_1 = __importStar(require("react"));
const gif_js_1 = __importDefault(require("gif.js"));
const AssetsLibrary_1 = __importDefault(require("./AssetsLibrary"));
const ParticleField_1 = require("@/components/art/generators/ParticleField");
const art_adapter_1 = require("@/lib/art-adapter");
...
function ModernSceneBuilder({ initialCommands = [], onSave }) {
    const canvasRef = (0, react_1.useRef)(null);
    const fileInputRef = (0, react_1.useRef)(null);
    const imageCache = (0, react_1.useRef)(new Map());
    const [elements, setElements] = (0, react_1.useState)([]);
    const [selectedElement, setSelectedElement] = (0, react_1.useState)(null);
    const [selectedElements, setSelectedElements] = (0, react_1.useState)(new Set());
    ...
    const [resizeMode, setResizeMode] = (0, react_1.useState)(null);
    const [resizeStart, setResizeStart] = (0, react_1.useState)(null);
    const [canvasScale, setCanvasScale] = (0, react_1.useState)(1);
    const [showGrid, setShowGrid] = (0, react_1.useState)(true);
    const [snapToGrid, setSnapToGrid] = (0, react_1.useState)(true);
    const [currentPalette, setCurrentPalette] = (0, react_1.useState)('modern');
    const [isPlaying, setIsPlaying] = (0, react_1.useState)(false);
    const [animationFrame, setAnimationFrame] = (0, react_1.useState)(0);
    ...
    const [artMode, setArtMode] = (0, react_1.useState)(false);
    const [isDraggingOver, setIsDraggingOver] = (0, react_1.useState)(false);
    const [isSelecting, setIsSelecting] = (0, react_1.useState)(false);
    const [selectionRect, setSelectionRect] = (0, react_1.useState)(null);
    const [selectionStart, setSelectionStart] = (0, react_1.useState)(null);
    const [lockAspectRatio, setLockAspectRatio] = (0, react_1.useState)(false);
    // Error handling and notifications
    const [errors, setErrors] = (0, react_1.useState)([]);
    const [isLoading, setIsLoading] = (0, react_1.useState)(false);
    const [loadingMessage, setLoadingMessage] = (0, react_1.useState)('');
    // Performance optimization flags
    const [needsRender, setNeedsRender] = (0, react_1.useState)(true);
    const renderTimeoutRef = (0, react_1.useRef)(null);
    const lastRenderTime = (0, react_1.useRef)(0);
    // Add error to notification queue
    const addError = (0, react_1.useCallback)((message) => {
    ...
    // Show loading state
    const showLoading = (0, react_1.useCallback)((message) => {
        setIsLoading(true);
        setLoadingMessage(message);
    }, []);
    // Hide loading state
    const hideLoading = (0, react_1.useCallback)(() => {
        setIsLoading(false);
        setLoadingMessage('');
    }, []);
    // Memory management - clean up unused images
    const cleanupImageCache = (0, react_1.useCallback)(() => {
    ...
    (0, react_1.useEffect)(() => {
        if (initialCommands && initialCommands.length > 0) {
            const converted = initialCommands.map((cmd, index) => {
                const element = {
    ...
    const ungroupElements = () => {
        const groupsToUngroup = elements.filter(el => el.type === 'group' && selectedElements.has(el.id));
    ...
    const updateElement = (id, updates, saveHistory = true) => {
        ...
        const clicked = elements.slice().reverse().find(el => {
            switch (el.type) {
                ...
                case 'line':
                    ...
                    const t = Math.max(0, Math.min(1, (clickVec.x * lineVec.x + clickVec.y * lineVec.y) / (lineLen * lineLen)));
    ...
    // Handle file drop
    const handleFileDrop = (files, x, y) => {
    ...
    const requestRender = (0, react_1.useCallback)(() => {
        setNeedsRender(true);
    }, []);
        ...
        elements.forEach(element => {
            ...
            switch (element.type) {
                case 'text':
                    ...
                    ctx.textAlign = element.props.align;
                ...
                case 'image':
                    if (element.props.src) {
                        ...
                        if (img.complete && img.naturalWidth > 0) {
                            ...
                            ctx.fillRect(element.x - (element.props.width || 100) / 2, element.y - (element.props.height || 100) / 2, element.props.width || 100, element.props.height || 100);
    ...
    // Export as MP4 using FFmpeg (server-side)
    const exportMP4 = async () => {
        const canvas = canvasRef.current;
        if (!canvas || elements.length === 0)
            return;
        const exportButton = document.querySelector('[data-export-mp4]');
        if (exportButton) {
            ...
            for (let frame = 0; frame < totalFrames; frame++) {
                ...
                elements.forEach((element, index) => {
                    ...
                    switch (element.type) {
                        ...
                        case 'image':
                            if (element.props.src) {
                                const img = imageCache.current?.get(element.props.src);
                                if (img && img.complete && img.naturalWidth > 0) {
                                    const width = element.props.width || 100;
                                    const height = element.props.height || 100;
                                    bufferCtx.drawImage(img, element.x - width / 2, element.y - height / 2, width, height);
                                }
                            }
                            break;
                        case 'rect':
                            if (element.props.filled) {
                                bufferCtx.fillStyle = element.props.color;
                                bufferCtx.fillRect(element.x - element.props.width / 2, element.y - element.props.height / 2, element.props.width, element.props.height);
                            }
                            else {
                                bufferCtx.strokeStyle = element.props.color;
                                bufferCtx.lineWidth = element.props.borderWidth || 2;
                                bufferCtx.strokeRect(element.x - element.props.width / 2, element.y - element.props.height / 2, element.props.width, element.props.height);
                            }
                            break;
                        case 'circle':
                            bufferCtx.beginPath();
                ...
                if (frameCtx) {
                    frameCtx.drawImage(bufferCanvas, GLOW_BUFFER, GLOW_BUFFER, CANVAS_WIDTH, CANVAS_HEIGHT, 0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
    ...
    const exportVideo = async () => {
        ...
        if (!canvas || elements.length === 0)
            return;
        const exportButton = document.querySelector('[data-export-video]');
        if (exportButton) {
        ...
        try {
            ...
            if (!testCanvas.captureStream && !testCanvas.mozCaptureStream && !testCanvas.webkitCaptureStream) {
            ...
            if (canvas.captureStream) {
                ...
                stream = canvas.webkitCaptureStream(25);
            }
            else {
    ...
    const exportFrameSequence = async () => {
        ...
        if (!canvas || elements.length === 0)
            return;
        const exportButton = document.querySelector('[data-export-frames]');
        if (exportButton) {
                    ...
                    switch (element.type) {
                        ...
                        case 'rect':
                            if (element.props.filled !== false) {
                                ...
                                bufferCtx.fillRect(element.x - (element.props.width || 100) / 2, element.y - (element.props.height || 100) / 2, element.props.width || 100, element.props.height || 100);
                            }
                            else {
                                bufferCtx.strokeStyle = element.props.color || '#3B82F6';
                                bufferCtx.lineWidth = element.props.borderWidth || 2;
                                bufferCtx.strokeRect(element.x - (element.props.width || 100) / 2, element.y - (element.props.height || 100) / 2, element.props.width || 100, element.props.height || 100);
                            }
                            break;
                        case 'circle':
                            bufferCtx.beginPath();
                        ...
                        case 'arrow':
                            ...
                            bufferCtx.lineTo(endX - arrowSize * Math.cos(angle - Math.PI / 6), endY - arrowSize * Math.sin(angle - Math.PI / 6));
                            bufferCtx.moveTo(endX, endY);
                            bufferCtx.lineTo(endX - arrowSize * Math.cos(angle + Math.PI / 6), endY - arrowSize * Math.sin(angle + Math.PI / 6));
                            bufferCtx.stroke();
                            break;
                            ...
                            if (element.props.src) {
                                ...
                                if (element.props.artType === 'particle') {
                                    ...
                                    if (img && img.complete && img.naturalWidth > 0) {
                                        ...
                                        bufferCtx.fillRect(element.x - width / 2, element.y - height / 2, width, height);
    ...
    const exportGIF = async () => {
        ...
        const exportButton = document.querySelector('[data-export-gif]');
        try {
            ...
            for (let frame = 0; frame < totalFrames; frame++) {
                        ...
                        case 'image':
                            if (element.props.src) {
                                ...
                                if (element.props.artType === 'particle') {
                                    ...
                                    bufferCtx.save();
                                    // Background gradient
                                    const gradient = bufferCtx.createLinearGradient(centerX - width / 2, centerY - height / 2, centerX + width / 2, centerY + height / 2);
                                    ...
                                    for (let i = 0; i < particleCount; i++) {
                                        ...
                                        for (let j = i + 1; j < Math.min(i + 5, particleCount); j++) {
                                            ...
                                            const distance = Math.sqrt(Math.pow(particleX - particleX2, 2) +
                ...
                if (frameCtx) {
                    ...
                    frameCtx.drawImage(bufferCanvas, GLOW_BUFFER, // source x
    ...
    return (<div className="min-h-screen bg-gray-950">
      {/* Error Notifications */}
      {errors.length > 0 && (<div className="fixed top-4 right-4 z-50 space-y-2">
          {errors.map((error, index) => (<div key={error} className="bg-red-600 text-white px-4 py-3 rounded-lg shadow-lg flex items-center gap-2 animate-slide-in">
      ...
      {/* Loading Overlay */}
      {isLoading && (<div className="fixed inset-0 bg-black bg-opacity-50 z-40 flex items-center justify-center">
      ...
      <div className="bg-gray-900 border-b border-gray-800 px-4 py-3">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={exportPNG} className="px-3 py-1 bg-green-600 hover:bg-green-700 text-white rounded text-xs">
              Export PNG
            </button>
            <button onClick={exportGIF} data-export-gif className="px-3 py-1 bg-purple-600 hover:bg-purple-700 text-white rounded text-xs">
              Export GIF
            </button>
            <button onClick={exportVideo} data-export-video className="px-3 py-1 bg-green-600 hover:bg-green-700 text-white rounded text-xs" title="Export as WebM video">
              Export WebM
            </button>
            <button onClick={exportMP4} data-export-mp4 className="px-3 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs" title="Export as MP4 video (FFmpeg)">
              Export MP4
            </button>
            {onSave && (<button onClick={() => onSave(elements)} className="px-3 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs">
                Save Scene
              </button>)}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          {/* Left Side - Controls */}
          <div className="flex items-center gap-2">
            <select value={currentPalette} onChange={(e) => setCurrentPalette(e.target.value)} className="px-2 py-1 bg-gray-800 text-white rounded text-xs">
            ...
            <button onClick={() => setShowGrid(!showGrid)} className={`px-2 py-1 rounded text-xs ${showGrid ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400'}`}>
              Grid
            </button>

            <button onClick={() => setSnapToGrid(!snapToGrid)} className={`px-2 py-1 rounded text-xs ${snapToGrid ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400'}`}>
              Snap
            </button>

            <button onClick={groupElements} disabled={selectedElements.size < 2} className={`px-3 py-1 rounded text-xs ${selectedElements.size < 2
            ? 'bg-gray-800 text-gray-500 cursor-not-allowed'
            : 'bg-indigo-600 hover:bg-indigo-700 text-white'}`} title="Group selected elements (Cmd/Ctrl+G)">
              Group
            </button>

            <button onClick={ungroupElements} disabled={!elements.some(el => el.type === 'group' && selectedElements.has(el.id))} className={`px-3 py-1 rounded text-xs ${!elements.some(el => el.type === 'group' && selectedElements.has(el.id))
            ? 'bg-gray-800 text-gray-500 cursor-not-allowed'
            : 'bg-indigo-600 hover:bg-indigo-700 text-white'}`} title="Ungroup selected groups (Cmd/Ctrl+Shift+G)">
              Ungroup
            </button>
          </div>
          ...
          <div className="flex items-center bg-gray-800 rounded px-2 py-1">
            <button onClick={() => setCanvasScale(Math.max(0.1, canvasScale - 0.1))} className="px-2 py-1 text-gray-400 hover:text-white transition-colors" title="Zoom Out">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 12H4"/>
              </svg>
            </button>
            <input type="range" min="10" max="200" value={canvasScale * 100} onChange={(e) => setCanvasScale(Number(e.target.value) / 100)} className="w-20 mx-2"/>
            <span className="text-xs text-white font-medium min-w-[40px] text-center">
              {Math.round(canvasScale * 100)}%
            </span>
            <button onClick={() => setCanvasScale(Math.min(2, canvasScale + 0.1))} className="px-2 py-1 text-gray-400 hover:text-white transition-colors" title="Zoom In">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4"/>
              </svg>
            </button>
            <div className="border-l border-gray-700 ml-2 pl-2 flex gap-1">
              <button onClick={() => setCanvasScale(1)} className="text-xs px-2 py-1 text-gray-500 hover:text-white transition-colors">
                100%
              </button>
              <button onClick={() => {
            const container = document.getElementById('canvas-container');
            if (container) {
                const scale = Math.min((container.clientWidth - 64) / 1920, (container.clientHeight - 64) / 1080);
        ...
        }} className="text-xs px-2 py-1 text-gray-500 hover:text-white transition-colors" title="Reset View">
                Reset
              </button>
            </div>
          </div>
      ...
      <div className="flex" style={{ height: 'calc(100vh - 120px)' }}>
        ...
        <div className="bg-gray-900 border-r border-gray-800 overflow-y-auto flex-shrink-0" style={{ width: '320px', height: '100%' }}>
          ...
          <div className="flex border-b border-gray-800">
            <button onClick={() => setActiveTab('templates')} className={`flex-1 px-4 py-3 text-sm font-medium transition-colors ${activeTab === 'templates'
            ? 'text-blue-400 border-b-2 border-blue-400 bg-gray-800/50'
            : 'text-gray-400 hover:text-white'}`}>
              Elements
            </button>
            <button onClick={() => setActiveTab('assets')} className={`flex-1 px-4 py-3 text-sm font-medium transition-colors ${activeTab === 'assets'
            ? 'text-blue-400 border-b-2 border-blue-400 bg-gray-800/50'
            : 'text-gray-400 hover:text-white'}`}>
              Assets
            </button>
            <button onClick={() => setActiveTab('lab')} className={`flex-1 px-4 py-3 text-sm font-medium transition-colors relative ${activeTab === 'lab'
          ...
          {activeTab === 'templates' ? (<div className="space-y-4 p-4">
            {/* Add Elements */}
            <div>
              <h3 className="text-xs font-semibold text-gray-400 uppercase mb-2 tracking-wide">Add Elements</h3>
              <div className="grid grid-cols-3 gap-1">
                <button onClick={() => addElement('text')} className="p-2 bg-gray-800 hover:bg-gray-700 rounded text-xs text-white flex flex-col items-center gap-0.5 transition-colors">
                  <span className="text-lg">T</span>
                  <span className="text-[10px]">Text</span>
                </button>
                <button onClick={() => addElement('rect')} className="p-2 bg-gray-800 hover:bg-gray-700 rounded text-xs text-white flex flex-col items-center gap-0.5 transition-colors">
                  <span className="text-lg">□</span>
                  <span className="text-[10px]">Rect</span>
                </button>
                <button onClick={() => addElement('circle')} className="p-2 bg-gray-800 hover:bg-gray-700 rounded text-xs text-white flex flex-col items-center gap-0.5 transition-colors">
                  <span className="text-lg">○</span>
                  <span className="text-[10px]">Circle</span>
                </button>
                <button onClick={() => addElement('arrow')} className="p-2 bg-gray-800 hover:bg-gray-700 rounded text-xs text-white flex flex-col items-center gap-0.5 transition-colors">
                  <span className="text-lg">→</span>
                  <span className="text-[10px]">Arrow</span>
                </button>
                <button onClick={() => addElement('line')} className="p-2 bg-gray-800 hover:bg-gray-700 rounded text-xs text-white flex flex-col items-center gap-0.5 transition-colors">
                  <span className="text-lg">—</span>
            ...
            {selectedElement && (<div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input type="number" value={Math.round(selectedElement.x)} onChange={(e) => updateElement(selectedElement.id, { x: Number(e.target.value) })} className="w-full px-2 py-1 bg-gray-800 rounded text-sm text-white"/>
                  </div>
                  <div>
                    <label className="text-xs text-gray-500">Y</label>
                    <input type="number" value={Math.round(selectedElement.y)} onChange={(e) => updateElement(selectedElement.id, { y: Number(e.target.value) })} className="w-full px-2 py-1 bg-gray-800 rounded text-sm text-white"/>
                  </div>
                </div>
                ...
                {selectedElement.type === 'text' && (<>
                    <div>
                      ...
                      <input type="text" value={selectedElement.props.text} onChange={(e) => updateElement(selectedElement.id, {
                    ...
                    <div>
                      ...
                      <input type="range" min="12" max="144" value={selectedElement.props.size} onChange={(e) => updateElement(selectedElement.id, {
                ...
                {selectedElement.type === 'image' && (<>
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      <input type="checkbox" id="lockAspectRatio" checked={lockAspectRatio} onChange={(e) => setLockAspectRatio(e.target.checked)} className="rounded border-gray-600 bg-gray-800 text-blue-500 focus:ring-blue-500"/>
                    ...
                    <div>
                      ...
                      <input type="range" min="50" max="500" value={selectedElement.props.width || 100} onChange={(e) => {
                    ...
                    <div>
                      ...
                      <input type="range" min="50" max="500" value={selectedElement.props.height || 100} onChange={(e) => {
                ...
                {selectedElement.type !== 'image' && (<div>
                    <label className="text-xs text-gray-500">Color</label>
                    <input type="color" value={selectedElement.props.color} onChange={(e) => updateElement(selectedElement.id, {
                ...
                <div className="border-t border-gray-700 pt-3 mt-3">
                  ...
                  <div className="mb-2">
                    ...
                    <select value={selectedElement.animation?.type || 'fadeIn'} onChange={(e) => updateElement(selectedElement.id, {
                    animation: {
                  ...
                  <div className="mb-2">
                    ...
                    <input type="number" min="100" max="5000" step="100" value={selectedElement.animation?.duration || 1000} onChange={(e) => updateElement(selectedElement.id, {
                    animation: {
                        ...selectedElement.animation,
                  ...
                  <div className="mb-2">
                    ...
                    <input type="number" min="0" max="3000" step="50" value={selectedElement.animation?.delay || 0} onChange={(e) => updateElement(selectedElement.id, {
                    animation: {
                        ...selectedElement.animation,
                  ...
                  <div className="mb-2">
                    ...
                    <select value={selectedElement.animation?.easing || 'ease-out'} onChange={(e) => updateElement(selectedElement.id, {
                    animation: {
                        ...selectedElement.animation,
                ...
                <div className="flex gap-2">
                  <button onClick={duplicateElement} className="flex-1 px-3 py-1.5 bg-gray-800 hover:bg-gray-700 rounded text-sm text-white">
                    Duplicate
                  </button>
                  <button onClick={deleteElement} className="flex-1 px-3 py-1.5 bg-red-600 hover:bg-red-700 rounded text-sm text-white">
                    Delete
                  </button>
                </div>
              </div>)}
            ...
            <div>
              ...
              <div className="space-y-1 max-h-64 overflow-y-auto">
                {elements.slice().reverse().map((element, index) => (<button key={element.id} onClick={() => setSelectedElement(element)} className={`w-full px-3 py-2 rounded text-left text-xs transition-colors ${selectedElement?.id === element.id
                    ? 'bg-blue-600 text-white'
                    : 'bg-gray-800 hover:bg-gray-700 text-gray-300'}`}>
                    <div className="flex items-center justify-between">
                      <span className="capitalize font-medium">{element.type}</span>
                      {element.type === 'text' && (<span className="text-xs opacity-60 truncate ml-2 max-w-[100px]">
                          {element.props.text}
                        </span>)}
                    </div>
                  </button>))}
                {elements.length === 0 && (<div className="text-center text-gray-500 text-xs py-4">
                    No elements yet
                  </div>)}
              </div>
            </div>
        ...
        /* Assets Library Tab */
        <AssetsLibrary_1.default onAssetSelect={handleAssetSelect}/>) : (
        ...
        <div className="space-y-4 p-4">
              ...
              <div className="flex items-center justify-between p-3 bg-gray-800 rounded-lg">
                ...
                <button onClick={() => setArtMode(!artMode)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${artMode ? 'bg-purple-600' : 'bg-gray-600'}`}>
                  <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${artMode ? 'translate-x-6' : 'translate-x-1'}`}/>
                </button>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <button onClick={() => art_adapter_1.ArtConfig.setQuality('low')} className="flex-1 px-2 py-1 text-xs bg-gray-800 hover:bg-gray-700 rounded text-white">
                    Low
                  </button>
                  <button onClick={() => art_adapter_1.ArtConfig.setQuality('medium')} className="flex-1 px-2 py-1 text-xs bg-gray-800 hover:bg-gray-700 rounded text-white">
                    Medium
                  </button>
                  <button onClick={() => art_adapter_1.ArtConfig.setQuality('high')} className="flex-1 px-2 py-1 text-xs bg-gray-800 hover:bg-gray-700 rounded text-white">
                    High
                  </button>
                </div>
              </div>
              ...
              {artMode && (<div className="space-y-3">
                  ...
                  <div className="bg-gray-800 rounded-lg p-3">
                    ...
                    <div className="mb-3 rounded-lg overflow-hidden bg-gray-900">
                      <ParticleField_1.ParticleField width={280} height={150} particleCount={50} speed={0.5} interactive={true}/>
                    </div>

                    <button onClick={() => {
                    const particleElement = new ParticleField_1.ParticleFieldArtElement({
                        width: 400,
                        height: 300,
                        particleCount: 100,
                    ...
                    const canvasElement = art_adapter_1.ArtAdapter.convertToCanvasElement(particleElement);
                  ...
                  <div className="space-y-2">
                    <button disabled className="w-full px-3 py-2 bg-gray-800/50 rounded text-sm text-gray-500 flex items-center justify-between cursor-not-allowed">
        ...
        <div id="canvas-container" className="flex-1 bg-gradient-to-br from-gray-900 to-gray-950 flex items-center justify-center overflow-auto" style={{ height: '100%', padding: '40px' }}>
          <div className="relative rounded-lg shadow-2xl" style={{
            ...
            <canvas ref={canvasRef} width={CANVAS_WIDTH} height={CANVAS_HEIGHT} className="rounded-lg cursor-move" style={{
            background: '#0F172A',
            boxShadow: '0 20px 60px rgba(0,0,0,0.5)'
        }} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} onWheel={handleWheel} onDragOver={handleCanvasDragOver} onDragLeave={handleCanvasDragLeave} onDrop={handleCanvasDrop}/>

            {/* Drag Over Indicator */}
            {isDraggingOver && (<div className="absolute inset-0 bg-blue-500 bg-opacity-10 border-2 border-blue-500 border-dashed rounded-lg pointer-events-none flex items-center justify-center">
