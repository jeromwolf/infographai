...
exports.default = ProfessionalSceneBuilder;
const react_1 = __importStar(require("react"));
const gif_js_1 = __importDefault(require("gif.js"));
function ProfessionalSceneBuilder({ initialCommands = [], onSave }) {
    ...
    // Canvas state
    const [canvasZoom, setCanvasZoom] = (0, react_1.useState)(60);
    ...
    // Export state
    const [isExportingGIF, setIsExportingGIF] = (0, react_1.useState)(false);
    // Canvas dimensions
    ...
    return (<div className="h-screen bg-gray-950 text-white flex flex-col">
      {/* Top Toolbar */}
      <div className="bg-gray-900 border-b border-gray-800 px-4 py-2 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => onSave && onSave(elements)} className="px-3 py-1 bg-gray-800 hover:bg-gray-700 rounded text-sm">
              Save
            </button>
            <button onClick={exportAsPNG} className="px-3 py-1 bg-green-600 hover:bg-green-700 rounded text-sm">
              Export PNG
            </button>
            <button onClick={exportAsGIF} disabled={isExportingGIF || elements.length === 0} className="px-3 py-1 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 rounded text-sm">
          ...
          <div className="flex items-center gap-1">
            <button onClick={undo} disabled={historyIndex <= 0} className="p-1 hover:bg-gray-700 rounded disabled:opacity-50" title="Undo (Ctrl+Z)">
              ↶
            </button>
            <button onClick={redo} disabled={historyIndex >= history.length - 1} className="p-1 hover:bg-gray-700 rounded disabled:opacity-50" title="Redo (Ctrl+Shift+Z)">
              ↷
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setCanvasZoom(Math.max(25, canvasZoom - 10))} className="px-2 py-1 bg-gray-800 hover:bg-gray-700 rounded text-sm">
              -
            </button>
            <span className="text-sm w-12 text-center">{canvasZoom}%</span>
            <button onClick={() => setCanvasZoom(Math.min(200, canvasZoom + 10))} className="px-2 py-1 bg-gray-800 hover:bg-gray-700 rounded text-sm">
              +
            </button>
          </div>
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          <div className="p-4">
            {leftPanelOpen ? (<>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => addElement('text')} className="px-3 py-2 bg-green-600 hover:bg-green-700 rounded text-sm">
                    Text
                  </button>
                  <button onClick={() => addElement('rect')} className="px-3 py-2 bg-blue-600 hover:bg-blue-700 rounded text-sm">
                    Rectangle
                  </button>
                  <button onClick={() => addElement('circle')} className="px-3 py-2 bg-red-600 hover:bg-red-700 rounded text-sm">
                    Circle
                  </button>
                  <button onClick={() => addElement('arrow')} className="px-3 py-2 bg-yellow-600 hover:bg-yellow-700 rounded text-sm">
                    Arrow
                  </button>
                </div>
                ...
                <div className="space-y-1 max-h-96 overflow-y-auto">
                  {elements.map((element) => (<div key={element.id} onClick={() => setSelectedElement(element)} className={`p-2 rounded cursor-pointer text-sm ${selectedElement?.id === element.id
                    ...
                    deleteElement(element.id);
                      ...
                      {element.type === 'text' && (<div className="text-xs text-gray-400 mt-1 truncate">
                          {element.props.text}
                        </div>)}
                    </div>))}
                </div>
        ...
        <div className="flex-1 flex items-center justify-center bg-gray-900 overflow-auto p-8">
          <div style={{
            ...
            <canvas ref={canvasRef} width={CANVAS_WIDTH} height={CANVAS_HEIGHT} className="border border-gray-700 shadow-2xl" style={{
          ...
          <div className="p-4">
            {rightPanelOpen ? (<>
                ...
                <div className="flex gap-2 mb-4">
                  <button onClick={() => setActiveTab('properties')} className={`px-3 py-1 rounded text-sm ${activeTab === 'properties'
                ? 'bg-blue-600'
                : 'bg-gray-800 hover:bg-gray-700'}`}>
                    Properties
                  </button>
                  <button onClick={() => setActiveTab('animation')} className={`px-3 py-1 rounded text-sm ${activeTab === 'animation'
                ? 'bg-blue-600'
                : 'bg-gray-800 hover:bg-gray-700'}`}>
                    Animation
                  </button>
                ...
                {selectedElement && activeTab === 'properties' && (<div className="space-y-4">
                    <h3 className="text-sm font-bold text-gray-400">POSITION</h3>
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="text-xs text-gray-500">X</label>
                        <input type="number" value={selectedElement.x} onChange={(e) => updateElement(selectedElement.id, { x: parseInt(e.target.value) })} className="w-full px-2 py-1 bg-gray-800 rounded text-sm"/>
                      </div>
                      <div>
                        <label className="text-xs text-gray-500">Y</label>
                        <input type="number" value={selectedElement.y} onChange={(e) => updateElement(selectedElement.id, { y: parseInt(e.target.value) })} className="w-full px-2 py-1 bg-gray-800 rounded text-sm"/>
                      </div>
                    </div>
                    ...
                    {selectedElement.type === 'text' && (<>
                        ...
                        <div>
                          ...
                          <input type="range" min="12" max="120" value={selectedElement.props.size} onChange={(e) => updateElement(selectedElement.id, {
                    ...
                    {(selectedElement.type === 'rect' || selectedElement.type === 'circle') && (<>
                        ...
                        <div className="flex items-center gap-2">
                          <input type="checkbox" checked={selectedElement.props.filled} onChange={(e) => updateElement(selectedElement.id, {
                    ...
                    <div className="flex gap-2 mt-4">
                      <button onClick={duplicateElement} className="px-3 py-1 bg-gray-800 hover:bg-gray-700 rounded text-sm">
                        Duplicate
                      </button>
                      <button onClick={() => deleteElement(selectedElement.id)} className="px-3 py-1 bg-red-600 hover:bg-red-700 rounded text-sm">
                        Delete
                      </button>
                    </div>
                  </div>)}

                {selectedElement && activeTab === 'animation' && (<div className="space-y-4">
                    ...
                    <div>
                      ...
                      <input type="number" value={selectedElement.animation?.duration || 1000} onChange={(e) => updateElement(selectedElement.id, {
                    animation: {
                        ...selectedElement.animation,
                    ...
                    <div>
                      ...
                      <input type="number" value={selectedElement.animation?.delay || 0} onChange={(e) => updateElement(selectedElement.id, {
                    animation: {
                        ...selectedElement.animation,
                    ...
                    <div>
                      ...
                      <select value={selectedElement.animation?.easing || 'ease'} onChange={(e) => updateElement(selectedElement.id, {
                    animation: {
                        ...selectedElement.animation,
                ...
                {!selectedElement && (<div className="text-gray-500 text-sm">
                    Select an element to edit properties
                  </div>)}
              </>) : (<button onClick={() => setRightPanelOpen(true)} className="p-1">
                ←
              </button>)}
          </div>
        </div>
