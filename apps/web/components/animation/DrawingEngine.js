...
function DrawingEngine({ commands, width = 1200, height = 700, speed = 1, autoPlay = true, loop = false, onComplete }) {
    ...
    const findCenterForText = (x, y, commandIndex, text) => {
    ...
    const drawAnimatedText = (ctx, text, x, y, progress, size = 24, color = COLORS.white, commandIndex = -1) => {
        if (progress <= 0)
            return;
    ...
    const executeCommand = (0, react_1.useCallback)((ctx, command, progress, commandIndex = -1) => {
        ...
        switch (command.type) {
            ...
            case 'circle':
                ...
                break;
            case 'text':
                drawAnimatedText(ctx, command.text, command.x, command.y, progress, command.size, command.color, commandIndex);
                break;
            case 'arrow':
    ...
    return (<div className="relative rounded-lg overflow-hidden shadow-2xl" style={{ maxWidth: `${width}px`, maxHeight: `${height}px` }}>
      <canvas ref={canvasRef} className="block w-full h-auto" style={{
