...
function GeoMap({ points, routes = [], width = 1200, height = 800, center = [117, 10], // South China Sea center
scale = 1200 }) {
    const svgRef = (0, react_1.useRef)(null);
    const [worldData, setWorldData] = (0, react_1.useState)(null);
    ...
    (0, react_1.useEffect)(() => {
        ...
        fetch('https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json')
            ...
            .then(data => {
            setWorldData((0, topojson_client_1.feature)(data, data.objects.countries));
        });
    }, []);
    (0, react_1.useEffect)(() => {
    ...
    return (<div className="relative bg-gray-900 rounded-lg overflow-hidden">
      ...
      {selectedPoint && (<div className="absolute bottom-4 left-4 bg-black bg-opacity-90 text-white p-4 rounded-lg">
          <h4 className="font-bold text-lg">{selectedPoint.name}</h4>
          {selectedPoint.activity && (<p className="text-sm text-gray-300 mt-1">Activity: {selectedPoint.activity}</p>)}
          <p className="text-xs text-gray-400 mt-2">
            Coordinates: {selectedPoint.coordinates[1].toFixed(2)}°N, {selectedPoint.coordinates[0].toFixed(2)}°E
          </p>
          <button onClick={() => setSelectedPoint(null)} className="mt-2 text-xs text-blue-400 hover:text-blue-300">
            Close
          </button>
        </div>)}
