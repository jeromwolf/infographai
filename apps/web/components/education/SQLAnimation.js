...
exports.default = SQLAnimation;
const react_1 = __importStar(require("react"));
function SQLAnimation({ width = 1200, height = 700 }) {
    ...
    return (<div className="relative bg-black rounded-lg overflow-hidden" style={{ width, height }}>
      <svg ref={svgRef} width={width} height={height} className="absolute inset-0">
        ...
        <g className={`transition-opacity duration-500 ${animationStep >= 0 ? 'opacity-100' : 'opacity-0'}`}>
          {/* Cylinder layers */}
          {[0, 1, 2].map(i => (<g key={i}>
              <ellipse cx="150" cy={200 + i * 40} rx="80" ry="20" fill="#3b82f6" stroke="#3b82f6" strokeWidth="2"/>
              {i < 2 && (<>
                  <line x1="70" y1={200 + i * 40} x2="70" y2={240 + i * 40} stroke="#3b82f6" strokeWidth="2"/>
                  <line x1="230" y1={200 + i * 40} x2="230" y2={240 + i * 40} stroke="#3b82f6" strokeWidth="2"/>
                </>)}
            </g>))}
          <text x="150" y="350" fill="white" fontSize="28" fontFamily={handwritingFont} textAnchor="middle">
            SQL Database
          </text>
        </g>
        ...
        <g className={`transition-opacity duration-500 ${animationStep >= 1 ? 'opacity-100' : 'opacity-0'}`}>
          <path d="M 230 200 Q 300 150 400 150" stroke="#4ade80" strokeWidth="3" fill="none" markerEnd="url(#arrowhead)"/>
          <text x="280" y="130" fill="#4ade80" fontSize="24" fontFamily={handwritingFont} transform="rotate(-15 280 130)">
            Structured database
          </text>
        </g>
        ...
        {/* Table rows */}
        {animationStep >= 3 && (<g className="transition-opacity duration-500 opacity-100">
        ...
        {animationStep >= 5 && (<g className="transition-opacity duration-500 opacity-100">
        ...
        {animationStep >= 6 && (<g className="transition-opacity duration-500 opacity-100">
        ...
        {animationStep >= 7 && (<g className="transition-opacity duration-500 opacity-100">
        ...
        {animationStep >= 4 && (<g className="transition-opacity duration-500 opacity-100">
            <path d="M 790 190 Q 850 190 850 240" stroke="#ef4444" strokeWidth="3" fill="none" markerEnd="url(#arrowhead-red)"/>
        ...
        {/* SQL Query */}
        {animationStep >= 8 && (<g className="transition-opacity duration-500 opacity-100">
        ...
        {showResults && (<g className="animate-pulse">
            <path d="M 580 305 L 800 305" stroke="#4ade80" strokeWidth="3" strokeDasharray="5,5"/>
            <path d="M 800 305 Q 900 305 950 350" stroke="#4ade80" strokeWidth="3" fill="none" markerEnd="url(#arrowhead-green)"/>
            ...
            <path d="M 580 270 Q 520 350 560 420 Q 620 450 720 450 Q 820 450 900 420" stroke="#4ade80" strokeWidth="3" fill="none" markerEnd="url(#arrowhead-green)" strokeDasharray="5,5"/>
      ...
      <div className="absolute bottom-4 left-4 flex gap-2">
        ...
        <button onClick={() => setAnimationStep(8)} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 font-bold">
          Skip to End
        </button>
      </div>
    </div>);
