...
class AIAPI {
    ...
    // 생성된 시나리오 가져오기
    async getGeneratedScenario(id) {
        return this.request(`/api/ai/scenarios/${id}`);
    }
    // 시나리오 편집
    async updateGeneratedScenario(id, updates) {
        return this.request(`/api/ai/scenarios/${id}`, {
            method: 'PUT',
    ...
    // 편집된 시나리오로 비디오 재생성
    async regenerateVideo(scenarioId) {
