...
const particle_processor_1 = require("../src/particle-processor");
const subtitle_optimizer_1 = require("../src/subtitle-optimizer");
const tech_terms_1 = require("../src/tech-terms");
const subtitle_styler_1 = require("../src/subtitle-styler");
describe('ParticleProcessor', () => {
    describe('조사 선택', () => {
        test('받침 있는 단어', () => {
            expect(particle_processor_1.ParticleProcessor.selectParticle('개발', 'subject')).toBe('이');
            expect(particle_processor_1.ParticleProcessor.selectParticle('서버', 'topic')).toBe('는');
            expect(particle_processor_1.ParticleProcessor.selectParticle('Python', 'object')).toBe('를'); // Python은 영어라 받침 없음
        });
        test('받침 없는 단어', () => {
            expect(particle_processor_1.ParticleProcessor.selectParticle('데이터', 'subject')).toBe('가');
            expect(particle_processor_1.ParticleProcessor.selectParticle('API', 'topic')).toBe('는');
            expect(particle_processor_1.ParticleProcessor.selectParticle('코드', 'object')).toBe('를');
        });
        test('영어 단어 처리', () => {
            expect(particle_processor_1.ParticleProcessor.selectParticle('React', 'subject')).toBe('가');
            expect(particle_processor_1.ParticleProcessor.selectParticle('Docker', 'topic')).toBe('는');
        });
    });
    describe('문장 처리', () => {
        test('조사 자동 교정', () => {
            ...
            expect(particle_processor_1.ParticleProcessor.processText(input)).toBe(expected);
        });
    });
});
describe('SubtitleOptimizer', () => {
    const optimizer = new subtitle_optimizer_1.SubtitleOptimizer();
    test('긴 텍스트 분할', () => {
...
describe('TechTermDictionary', () => {
    test('용어 검색', () => {
        const react = tech_terms_1.TechTermDictionary.lookup('react');
        expect(react).toBeDefined();
        expect(react?.korean).toBe('리액트');
        expect(react?.preferKorean).toBe(true);
    });
    test('용어 번역', () => {
        expect(tech_terms_1.TechTermDictionary.translate('javascript')).toBe('자바스크립트');
        expect(tech_terms_1.TechTermDictionary.translate('vue')).toBe('Vue');
        expect(tech_terms_1.TechTermDictionary.translate('unknown')).toBe('unknown');
    });
    test('문장 내 용어 처리', () => {
        const input = 'javascript와 typescript를 사용하여 react 개발';
        const output = tech_terms_1.TechTermDictionary.processTermsInText(input);
...
describe('SubtitleStyler', () => {
    const styler = new subtitle_styler_1.SubtitleStyler();
    test('기본 스타일', () => {
...
describe('통합 테스트', () => {
    test('전체 자막 처리 파이프라인', () => {
        ...
        // 1. 기술 용어 처리
        const processed = tech_terms_1.TechTermDictionary.processTermsInText(text);
        expect(processed).toContain('리액트');
        expect(processed).toContain('도커');
        // 2. 조사 처리
        const withParticles = particle_processor_1.ParticleProcessor.processText(processed);
        // 3. 자막 분할
        const optimizer = new subtitle_optimizer_1.SubtitleOptimizer();
