...
exports.SubtitleOptimizer = void 0;
class SubtitleOptimizer {
     ...
     * 긴 텍스트를 자막 세그먼트로 분할
     */
    splitIntoSegments(text, totalDuration) {
    ...
    calculateDuration(text) {
        ...
        return Math.max(this.config.minDuration, Math.min(this.config.maxDuration, readingTime));
    }
    /**
     * 중요 키워드 추출
     */
    ...
    findNaturalBreaks(text) {
        ...
        breakPoints.forEach(pattern => {
            ...
            while ((match = pattern.exec(text)) !== null) {
                ...
                const isInNoBreakZone = noBreakZones.some(([start, end]) => pos >= start && pos < end);
                if (!isInNoBreakZone) {
                    breaks.push(pos);
                }
            }
...
exports.SubtitleOptimizer = SubtitleOptimizer;
