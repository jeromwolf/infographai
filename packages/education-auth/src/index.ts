...
// 플랜 정의
export const PLANS: Record<string, EducationPlan> = {
  // 일반 무료
  FREE: {
    name: '무료',
...
// 교육 인증 시스템
export class EducationAuthSystem {
  private jwtSecret: string;
  private verifiedUsers: Map<string, VerificationResult> = new Map();
  
  constructor(jwtSecret: string = process.env.JWT_SECRET || 'education-secret') {
   ...
   * 교육 플랜 승인
   */
  private async approveEducation(
    request: VerificationRequest,
    type: VerificationType
  ): Promise<VerificationResult> {
    // 기관명 추출
   ...
   * 문서 검증 (OCR 또는 수동)
   */
  private async verifyDocument(document: Buffer): Promise<boolean> {
   ...
   * SheerID API 연동
   */
  private async verifyViaSheerID(
    request: VerificationRequest
  ): Promise<boolean> {
    if (!process.env.SHEERID_API_KEY) return false;
   ...
   * 기관명 추출
   */
  private extractInstitution(email: string): string {
   ...
   * 토큰 검증
   */
  public verifyToken(token: string): VerificationResult | null {
    try {
   ...
   * 재인증 필요 체크
   */
  public needsReverification(email: string): boolean {
    const verification = this.verifiedUsers.get(email);
  ...
  private getInstitutionDomain(institution: string): string {
  ...
  public getStatistics(): {
    ...
    const stats = {
      ...
      byInstitution: new Map<string, number>(),
      byRole: new Map<EducationRole, number>()
    };
    
    this.verifiedUsers.forEach(user => {
      // 기관별
      const institution = user.institution || 'unknown';
      stats.byInstitution.set(
        institution,
        (stats.byInstitution.get(institution) || 0) + 1
      );
      
      // 역할별
      stats.byRole.set(
        user.role,
        (stats.byRole.get(user.role) || 0) + 1
      );
    });
